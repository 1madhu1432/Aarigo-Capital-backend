              ...
              if (Array.isArray(l.installments)) {
                l.installments.forEach((inst: any) => {
                  mappedEmis.push({
                    ...
                    principal: Number(inst.principalAmount) || 0,
                    interest: Number(inst.interestAmount) || 0,
                    status: inst.status === "PAID" ? "Paid" : inst.status === "PARTIAL" ? "Partial" : inst.status === "OVERDUE" ? "Overdue" : "Due",
                    paidDate: inst.paidAt ? String(inst.paidAt).slice(0, 10) : undefined,
        ...
        if (payRes.status === "fulfilled" && Array.isArray(payRes.value.data)) {
          ...
          if (rawPayments.length > 0) {
            const mappedPayments: Payment[] = rawPayments.map((p) => ({
              ...
              method: p.paymentMethod === "UPI" ? "UPI" : p.paymentMethod === "BANK_TRANSFER" ? "Bank" : "Cash",
        ...
        if (closingRes.status === "fulfilled" && Array.isArray(closingRes.value.data)) {
          ...
          if (rawClosings.length > 0) {
            const mappedClosings: DailyClosing[] = rawClosings.map((dc) => ({
              ...
              cash: Number(dc.cashAmount) || 0,
              upi: Number(dc.upiAmount) || 0,
              bank: Number(dc.bankAmount) || 0,
              transactions: Number(dc.transactionsCount || dc.totalTransactions) || 0,
              status: dc.status === "CLOSED" ? "Closed" : "Open",
